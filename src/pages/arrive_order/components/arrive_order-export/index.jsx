    ...
    exportExcel = (selIndex) => {
        ...
        if ( selIndex == '0' ) {
            ...
            if(!this.isEmpty(list)) {
                // 导出当前页
                window.open(`${GROBAL_HTTP_CTX}/arrive_order/toExportExcelAll?1=1&pageIndex=${pageIndex-1}&pageSize=${pageSize}`)
            }
        } else {
            if(!this.isEmpty(total)) {
                // 导出全部,将设置的页面数据条数设置为总数据条数
                window.open(`${GROBAL_HTTP_CTX}/arrive_order/toExportExcelAll?1=1&pageIndex=0&pageSize=${total}`)
            }
        }
