...
export default {
    ...
    initialState: {
        ...
        totalPages:1,
        total:0,
        childListmaterial:[],          //子表
        cacheArraymaterial:[],         //缓存数据
        delArraymaterial:[],
        detail:{},
        searchParam:{},
    ...
    effects: {
        ...
        async delItem(param,getState){
            ...
            let res=processData(await api.delArrive_order(param.param),'删除成功');
        ...
        async save(param,getState){//保存
            ...
            let res = processData(await api.saveArrive_order(param),'保存成功');
        ...
        async queryDetail(param,getState) {
            ...
            let {data:{detailMsg}}=await api.getDetail(param);
                    let childData = [...detailMsg.materialList] ;
                    let cacheArraymaterial = [];
                    let tempArraymaterial = [];
                    if(childData) {
                        childData.map((item)=>{
                            let temp = Object.assign({},item);
                            temp.uuid = setTimeout(function(){},1);
                            tempArraymaterial.push(temp);
                        })
                    }

                    // tempArray中修改参照字段
                    if(tempArraymaterial) {
                        tempArraymaterial.map((item)=>{
                            let temp = Object.assign({},item);
                            cacheArraymaterial.push(temp);
                        })
                    }

                    console.log("childList,cacheArray",tempArraymaterial,cacheArraymaterial);

                    await actions.arrive_order.updateState({
                        childListmaterial:tempArraymaterial,
                        cacheArraymaterial:cacheArraymaterial
                    })
